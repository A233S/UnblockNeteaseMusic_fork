...
const search = (info) => {
    ...
    const searchUrl = `https://music-api.gdstudio.xyz/api.php?types=search&source=joox&name=${keyword}&count=1&pages=1`;
...
const track = (info) => {
    const url = `https://music-api.gdstudio.xyz/api.php?types=url&source=joox&id=${info.id}&br=${
        ['999', '320'].slice(
